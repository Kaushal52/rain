...
rainfall_data = pd.read_csv("/Users/kaushalkhatu/Desktop/RAINFALL/RAIN.csv")
...
# Visualize Annual Rainfall and Anomalies
...
# Assuming 'rainfall_data' has 'YEAR' and 'ANNUAL' columns

rainfall_data['DATE'] = pd.to_datetime(rainfall_data['YEAR'], format='%Y')
annual_rainfall_ts = rainfall_data.set_index('DATE')['ANNUAL']

# Split data into train and test for model validation (optional)
train_data = annual_rainfall_ts[:int(0.8 * len(annual_rainfall_ts))]
test_data = annual_rainfall_ts[int(0.8 * len(annual_rainfall_ts)):]

# Fit an ARIMA model (p, d, q) - you may need to adjust these parameters
model = sm.tsa.ARIMA(train_data, order=(5, 1, 0))  # p=5, d=1, q=0 is a common choice for ARIMA
model_fit = model.fit()
...
# Create a future time index for plotting
forecast_index = pd.date_range(start=annual_rainfall_ts.index[-1] + pd.Timedelta(days=365), periods=forecast_steps, freq='Y')
...
# Add the actual rainfall data
fig_forecast.add_trace(go.Scatter(
    x=annual_rainfall_ts.index,
    y=annual_rainfall_ts,
    mode='lines',
...
fig_forecast.add_trace(go.Scatter(
    ...
    mode='lines',
    name='Forecasted Annual Rainfall',
    line=dict(color='red', dash='dash', width=2)
))

fig_forecast.update_layout(
    title='Annual Rainfall Forecast Using ARIMA',
    xaxis_title='Year',
    yaxis_title='Rainfall (mm)',
    template='plotly_white',
    height=500
